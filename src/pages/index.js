import * as React from "react"

import Layout from "../components/layout"
import SectionDiscover from "../components/section-discover/section-discover"

import Seo from "../components/seo"

import PersoDroiteCorps from "../images/img-accueil/hero-persos/perso droite/corp droite.webp"
import PersoDroiteTete from "../images/img-accueil/hero-persos/perso droite/tete droite.webp"
import PersoDroitemain from "../images/img-accueil/hero-persos/perso droite/main droite.webp"

import PersoGaucheCorps from "../images/img-accueil/hero-persos/perso gauche/corp gauche.webp"
import PersoGaucheTete from "../images/img-accueil/hero-persos/perso gauche/tete gauche.webp"
import PersoGauchemain from "../images/img-accueil/hero-persos/perso gauche/main gauche.webp"

import cristalCentre from "../images/img-accueil/crystaux flotants/crystal.webp"

import SectionSeparator from "../components/section-separator/section-separator"
import SectionPresentation from "../components/section-presentation/section-presentation"
import SectionSeparatorWarning from "../components/section-separator-warning/section-separator-warning"



const IndexPage = () => {

return(
  <Layout>
    <Seo title="Home" />
    <div className="hero--group">
      <div className="intro--bg">
        <div className="hero--bg--fit">
          <img src="data:image/png;base64," alt="" />
          <span className="normal"/>
          <span className="normal"/>
          <span className="inverted"/>
          <span className="normal"/>
          <span className="inverted"/>
          <span className="normal"/>
          <span className="inverted"/>
          <span className="normal"/>
          <span className="normal"/>
          <span className="normal"/>
          <span className="inverted"/>
          <span className="normal"/>
          <span className="inverted"/>
          <span className="normal"/>
          <span className="inverted"/>
          <span className="normal"/>
          <div className="hero--persos-droite">
            <div className="hero--persos-droite-container">
              <img className="hero--persos-droite--corps" src={PersoDroiteCorps} alt=""></img>
              <img className="hero--persos-droite--tete" src={PersoDroiteTete} alt=""></img>
              <img className="hero--persos-droite--main" src={PersoDroitemain} alt=""></img>
            </div>
          </div>
          <div className="hero--persos-gauche">
            <div className="hero--persos-gauche-container">
              <img className="hero--persos-gauche--corps" src={PersoGaucheCorps} alt=""></img>
              <img className="hero--persos-gauche--tete" src={PersoGaucheTete} alt=""></img>
              <img className="hero--persos-gauche--main" src={PersoGauchemain} alt=""></img>
            </div>
          </div>
          <div className="light-cristal"></div>
          <div className="hero--cristal-centre">
            <img className="hero--cristal-centre-pos" src={cristalCentre} alt=""></img>
          </div>
          <div className="hero--title-choose-side">
            <h1>Choose your side</h1>
          </div>
        </div>
      </div>
    </div>
    <SectionSeparator />
    <SectionPresentation />
    <SectionDiscover />
    <SectionSeparatorWarning />
    {/* <SectionRoadmap />
    <SectionIntro />
    <SectionFaction /> */}
    
  </Layout>
)
}

export default IndexPage
